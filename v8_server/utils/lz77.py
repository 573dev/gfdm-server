# Code directly from:
# https://github.com/ByteFun/bemaniutils/blob/bd467a9b732a25a1c8aba75106dc459fbdff61b0/
    ...
    def decompress_bytes(self) -> Generator[bytes, None, None]:
        ...
        while not self.eof:
            if self.pending_copy_amount > 0:
                # We had a backref that would have copied more data than we had
                # available in the ringbuffer (because every read, even a backref, adds
                # to the ringbuffer). So, since we read that last time and wrote it to
                # the backbuffer we are safe to read again.
                ...
                if flag == self.FLAG_COPY:
                    ...
                    while self.flags != 1 and (self.flags & 1) == self.FLAG_COPY:
                        # We would do a copy next time, so pop that flag and just add to
                        # our read amount
                        self.flags >>= 1
                        amount += 1
        ...
        while not self.eof:
            if self.left == 0:
                ...
                for _ in range(8):
                    ...
                    # Iterate over all spots where the first byte equals, and is in
                    # range.
                    earliest = max(0, self.bytes_written - self.ringlength)
                    possible_backref_locations: List[int] = [
                        absolute_pos
                        for absolute_pos in self.starts[
                            self.data[self.read_pos : (self.read_pos + 3)]
                        ]
                    ...
                    # we're going to write at least these three bytes, so append it to
                    # the output buffer.
...
class Lz77:
    ...
    def decompress(self, data: bytes) -> bytes:
        """
        Given a binary blob, return a new binary blob representing the decompressed
        data.
